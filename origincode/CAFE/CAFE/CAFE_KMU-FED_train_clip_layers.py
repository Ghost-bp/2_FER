"""
================================================================================
师兄原始 CAFE 代码 + CLIP 层数消融实验
================================================================================
基于: CAFE_KMU-FED_train_run.py (师兄原版, 仅路径修改)
新增: --clip_layers 参数, 控制 CLIP ViT Transformer 层数 (12/9/6/3)
其余代码 100% 保留师兄原版逻辑。
================================================================================
"""
import os
...
# ===================== CLIP 层数控制 =====================
# 用法: python CAFE_KMU-FED_train_clip_layers.py --clip_layers 9
CLIP_LAYERS = 12
if '--clip_layers' in sys.argv:
    idx = sys.argv.index('--clip_layers')
    CLIP_LAYERS = int(sys.argv[idx + 1])
    print(f"🔧 CLIP 层数设置为: {CLIP_LAYERS}")
else:
    print(f"🔧 CLIP 层数: {CLIP_LAYERS} (默认12层完整)")

# ===================== 加载 CLIP =====================
clip_model, preprocess = clip.load("clip/ViT-B-32.pt", device=device)

# 减少 CLIP 视觉编码器层数（截断 resblocks）
if CLIP_LAYERS < 12:
    visual = clip_model.visual
    original_layers = visual.transformer.layers
    old_resblocks = list(visual.transformer.resblocks.children())
    new_resblocks = old_resblocks[:CLIP_LAYERS]
    visual.transformer.resblocks = nn.Sequential(*new_resblocks)
    visual.transformer.layers = CLIP_LAYERS
    print(f"  CLIP 层数: {original_layers} → {CLIP_LAYERS}")
...
base_output = f"KMU-FED/output_kmu_fed_clip_l{CLIP_LAYERS}/"
...
def run_10fold():
    ...
    dataset = KMU_FED("../../../KMU-FED/KMU-FED", transform=val_transform)
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(unique_subjects)):
        ...
        train_dataset = KMU_FED("../../../KMU-FED/KMU-FED", transform=train_transform)
        val_dataset = dataset
